...
Скрипт мониторит логи Remnawave и автоматически блокирует IP-адреса при превышении лимита одновременных подключений с одного ключа на определённой ноде.
...
- [Установка](#-установка)
- [Использование CLI](#%EF%B8%8F-использование-cli)
...
- ✅ **Webhook уведомления** - интеграция с внешними системами оповещения
- ✅ **Автоматическая очистка логов** - предотвращение разрастания файлов
...
Скачайте репозиторий и запустите установку
```bash
...
|----------|--------------|----------|
| Путь к логу Remnawave | `/var/log/remnanode/access.log` | Access лог Remnawave |
| Максимум IP на ключ | `1` | Лимит одновременных IP |
| Время бана (минуты) | `30` | Длительность блокировки |
| Интервал проверки (сек) | `5` | Частота мониторинга логов |
| Интервал очистки лога (сек) | `3600` | Частота truncate лога |
| Название сервера | `VPN Server` | Отображается в webhook |
...
Файл: `/etc/fail2ban/jail.d/remnawave-limiter.conf`
...
maxretry = 3          # Количество нарушений для бана
findtime = 300        # Окно времени (5 минут)
bantime = 30m         # Длительность бана
```
...
# После изменения fail2ban конфигурации
...
# Посмотреть последние нарушения
limiter violations
...
$ limiter status
╔════════════════════════════════════════════════════════╗
║         Remnawave IP Limiter - Статус системы          ║
╚════════════════════════════════════════════════════════╝

📊 Сервис remnawave-limiter: active
🔒 Fail2ban: active

Статус jail 'remnawave-limiter':
|- Filter
|  |- Currently failed: 2
|  |- Total failed:     15
...
2025/11/28 12:00:00 │ user@example.com │ 1.2.3.4
2025/11/28 12:01:00 │ user@example.com │ 1.2.3.4
2025/11/28 12:02:00 │ user@example.com │ 1.2.3.4
```
...
$ limiter active
╔════════════════════════════════════════════════════════╗
║            Активные подключения (текущий лог)          ║
╚════════════════════════════════════════════════════════╝

📧 user1@example.com (2 IP)  # Красный - превышен лимит
   └─ 1.2.3.4
   └─ 5.6.7.8

📧 user2@example.com (1 IP)  # Зеленый - в пределах лимита
   └─ 9.10.11.12
```
...
```bash
$ limiter clear
⚠️  Это удалит все логи нарушений и Remnawave access log!
Продолжить? (yes/no): yes
✅ Лог нарушений очищен
✅ Access лог очищен
✅ Сервис перезапущен
```
...
При блокировке IP отправляется POST запрос с JSON:
...
```json
{
  ...
  "email": "ofgoak4kal3",
  "reason": "подключение к локации с 2 IP (лимит: 1)",
...
| Поле | Тип | Описание |
|------|-----|----------|
| `server` | string | Название сервера (из конфига) |
| `ban_duration_minutes` | int | Длительность бана в минутах |
| `ip_masked` | string | Маскированный IP (123.45.\*\*.\*\*) |
| `ip_full` | string | Полный IP адрес |
| `email` | string | Email ключа |
| `reason` | string | Причина блокировки |
| `timestamp` | string | ISO 8601 timestamp |
| `active_ip_count` | int | Количество активных IP |
| `limit` | int | Разрешенный лимит IP |
...
```bash
# Проверить что access лог существует
ls -la /var/log/remnanode/access.log

# Проверить формат лога
tail -5 /var/log/remnanode/access.log

# Проверить что скрипт обрабатывает лог
journalctl -u remnawave-limiter -f

# Проверить violations лог
tail -20 /var/log/remnawave-limiter/access-limiter.log
```

### Ложные срабатывания при переключении сети

Это нормально если переключение происходит быстро. Проверьте:

```bash
# Текущие настройки fail2ban
fail2ban-client get remnawave-limiter maxretry   # Должно быть 3
fail2ban-client get remnawave-limiter findtime   # Должно быть 300

# Если значения другие - исправьте в конфиге
...
Скрипт не логирует одно и то же нарушение чаще раза в минуту. Это значит:
- Если IP превышает лимит → логируется 1 раз
- Через 60 секунд, если все еще превышает → логируется снова
- Всего за 5 минут может быть максимум 5 записей Found
- Для бана нужно 3 Found → минимум 2 минуты до бана

### Можно ли использовать на нескольких серверах?

Да! Установите на каждый сервер и используйте разные `SERVER_NAME`:
```bash
# Сервер 1
SERVER_NAME=Вильнюс

# Сервер 2
SERVER_NAME=Амстердам
```

Webhook будет различать серверы по полю `server`.
...
# Перезагрузить systemd и fail2ban
...
- Основано на [fail2ban](https://github.com/fail2ban/fail2ban)
