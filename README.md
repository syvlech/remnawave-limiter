...
Миграция данных не нужна — достаточно `docker compose pull && docker compose up -d`. Но проверка конфигурации стала строгой, и сервис **откажется стартовать**, если в `.env` есть:

- **опечатка в числовом или булевом параметре** (`CHECK_INTERVAL=6O`, `AUTO_NOTIFY_SOFT=yes`). Раньше такое значение молча подменялось значением по умолчанию, и лимитер работал не с теми порогами, что задал администратор. Теперь при старте выводится список всех некорректных параметров сразу;
- **отрицательное значение** у `TOLERANCE`, `TOLERANCE_MULTIPLIER`, `USER_CACHE_TTL`, `DEFAULT_DEVICE_LIMIT`, `AUTO_DISABLE_DURATION`, `IGNORE_DURATION`;
- **`REMNAWAVE_API_URL` без схемы** (`panel.example.com` вместо `https://panel.example.com`) — раньше адрес принимался, а падал уже каждый запрос к панели;
- **нечисловой `WHITELIST_USER_IDS`** — UUID, оставшийся от версий до 4.0, никогда бы не совпал с ID пользователя и молча не работал;
- **неизвестная таймзона, язык, `LOG_LEVEL` или `LOG_FORMAT`**.

Причина всегда в первых строках `docker compose logs limiter`.

Заметные изменения в поведении:

- **Логи** переработаны под `docker compose logs -f`: одна сводная строка на каждый цикл проверки (`Проверка  nodes=2/2 took=308ms users=17 violations=1`), единый формат и никаких дублей от библиотеки Telegram. Появились `LOG_LEVEL` (меняется на лету через `/settings`) и `LOG_FORMAT=json` для сборщиков логов.
- **Восстановление по таймеру** больше не теряет пользователя, если панель была недоступна в момент срабатывания: ID возвращается в очередь, а после исчерпания попыток админы получают уведомление в чат.
- **`/healthz`** считает проверку успешной только если удалось опросить хотя бы одну ноду.
- **Счётчик «нарушений за 24ч»** теперь действительно истекает через 24 часа (раньше у постоянного нарушителя он не сбрасывался и рос неограниченно).
- **Запросы к панели** переживают `429` и `Retry-After` — раньше рейт-лимит срывал всю проверку.
- **Webhook** повторяет доставку при временных сбоях и добавляет заголовок `X-Timestamp`.
